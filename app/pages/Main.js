...
export default class Main extends Component {
    ...
    render() {
        /*let calendars = this.state.user.calendarArray.map((val, key) => {
            return <Calendar key={key} keyVal={key} val={val}/>
        });*/
        ...
        return (
            <View style={styles.container}>
                ...
                <Tabs>
                    <Tab heading='Calendar' style={styles.tab1}>
    ...
    deleteNote(key){
        ...
        Alert.alert(
            'Delete diary',
            'You sure you want delete this diary?',
            [
                {text: 'yes', onPress: () => {
                    JSON.stringify(removedData);
                    const removedData = this.state.user.noteArray.splice(key, 1);
